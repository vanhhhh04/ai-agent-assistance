...
ROOT=$(curl -sk -H "${AUTH_HDR}" "${NIFI}/process-groups/root" | python3 -c "import json,sys;print(json.load(sys.stdin)['id'])")
echo "  root PG: ${ROOT}"

echo ""
echo "Step 2: stop all processors in root PG (bulk)"
VER=$(curl -sk -H "${AUTH_HDR}" "${NIFI}/process-groups/${ROOT}" | python3 -c "import json,sys;print(json.load(sys.stdin)['revision']['version'])")
...
echo "${CONNECTIONS_JSON}" | python3 -c "
